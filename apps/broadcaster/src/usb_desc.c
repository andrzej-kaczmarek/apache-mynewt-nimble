...
//    TUD_AUDIO1_STEREO_MICROPHONE_DESCRIPTOR(ITF_NUM_AUDIO_AC, 0x80 | ISO_EP, 192, 1, 48000),
//    TUD_AUDIO_MIC_DESCRIPTOR(ITF_NUM_AUDIO_AC, 0, 2, 16, 0x80 | ISO_EP, 96),
    ...
    TUD_AUDIO_SPEAKER_STEREO_FB_DESCRIPTOR(ITF_NUM_AUDIO_AC, 0, CFG_TUD_AUDIO_N_BYTES_PER_SAMPLE_RX, 8 * CFG_TUD_AUDIO_N_BYTES_PER_SAMPLE_RX, ISO_EP, CFG_TUD_AUDIO_EPSIZE_OUT, 0x80 | ISO_EP, 1),
#else
    TUD_AUDIO_SPEAKER_STEREO_DESCRIPTOR(ITF_NUM_AUDIO_AC, 0, CFG_TUD_AUDIO_N_BYTES_PER_SAMPLE_RX, 8 * CFG_TUD_AUDIO_N_BYTES_PER_SAMPLE_RX, ISO_EP, CFG_TUD_AUDIO_EPSIZE_OUT),
//    TUD_AUDIO_SPEAKER_DESCRIPTOR_FLOAT(ITF_NUM_AUDIO_AC, 0, ISO_EP, CFG_TUD_AUDIO_EPSIZE_OUT),
#endif
...
#else
    TUD_AUDIO_SPEAKER_MONO_DESCRIPTOR(ITF_NUM_AUDIO_AC, 0, CFG_TUD_AUDIO_N_BYTES_PER_SAMPLE_RX, 8 * CFG_TUD_AUDIO_N_BYTES_PER_SAMPLE_RX, ISO_EP, CFG_TUD_AUDIO_EPSIZE_OUT),
#endif
#endif
};
...
tud_audio_set_req_entity_cb(uint8_t rhport, tusb_control_request_t const * p_request, uint8_t *pBuff)
{
    ...
    if (request->bEntityID == 2 && request->bControlSelector == AUDIO_FU_CTRL_VOLUME &&
        ...
        return true;
    } else if (request->bEntityID == 4 && request->bControlSelector == AUDIO_CS_CTRL_SAM_FREQ && request->bRequest == AUDIO_CS_REQ_CUR) {
        // Ignore value but accept request
        ...
        __BKPT(1);
//        console_printf("Set request not handled, entity = %d, selector = %d, request = %d",
...
tud_audio_get_req_entity_cb(uint8_t rhport, tusb_control_request_t const * p_request)
{
    ...
    if (request->bEntityID == 4 && request->bControlSelector == AUDIO_CS_CTRL_SAM_FREQ) {
        ...
        __BKPT(1);
//        console_printf("Get request not handled, entity = %d, selector = %d, request = %d",
//                       request->bEntityID, request->bControlSelector, request->bRequest);
    }
    return false;
}
...
serial_to_usb_desc_string(uint16_t *desc, size_t size)
{
    if (serial_number[0] == 0) {
        uint64_t serial = 1;
        /* It probably means that serial was not provisioned return 0s */
...
void
usb_audio_sample_rate_cb_set(usb_audio_sample_rate_cb_t cb)
{
    sample_rate_cb = cb;
}

uint32_t
usb_audio_sample_rate_get(void)
{
